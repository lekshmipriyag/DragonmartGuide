...
if (!function_exists("wpnetbase_setval_or_getoption")) {
    function wpnetbase_setval_or_getoption($options, $key, $def_key)
    {
...
if (!function_exists("wpnetbase_get_selected")) {
    function wpnetbase_get_selected($option, $key) {
        return isset($option['selected-'.$key])?$option['selected-'.$key]:array();
    }
}

if (!function_exists("wpnetbase_keyword_count_sort")) {
    function wpnetbase_keyword_count_sort($first, $sec) {
        return $sec[1] - $first[1];
    }
}

if (!function_exists("wpnetbase_get_stylesheet")) {
    function wpnetbase_get_stylesheet($dir, $id, $style) {
        ob_start();
...
if (!function_exists("wpnetbase_update_stylesheet")) {
    function wpnetbase_update_stylesheet($dir, $id, $style) {
        $out = wpnetbase_get_stylesheet($dir, $id, $style);
        if (isset($style['css_compress']) && $style['css_compress'] == true)
            $out = wpnetbase_css_compress($out);
        return @file_put_contents($dir."style".$id.".css", $out, FILE_TEXT);
    }
}

if (!function_exists("wpnetbase_parse_params")) {
    function wpnetbase_parse_params($params) {
...
if (!function_exists("wpnetbase_admin_hex2rgb")) {
    function wpnetbase_admin_hex2rgb($color)
    {
...
if (!function_exists("wpnetbase_four_to_string")) {
    function wpnetbase_four_to_string($data) {
...
if (!function_exists("wpnetbase_box_shadow_css")) {
    function wpnetbase_box_shadow_css($css) {
        $css = str_replace("\n", "", $css);
...
if (!function_exists("wpnetbase_gradient_css")) {
    function wpnetbase_gradient_css($data, $print=true)
    {
        ...
        $type = $matches[1];
        $deg = $matches[2];
        $color1 = wpnetbase_admin_hex2rgb($matches[3]);
        $color2 = wpnetbase_admin_hex2rgb($matches[4]);
...
if (!function_exists("wpnetbase_gradient_css_rgba")) {
    function wpnetbase_gradient_css_rgba($data, $print=true)
    {
...
if (!function_exists("wpnetbase_border_width")) {
    function wpnetbase_border_width($css)
    {
        $css = str_replace("\n", "", $css);
...
if (!function_exists("wpnetbase_width_from_px")) {
    function wpnetbase_width_from_px($css)
    {
        $css = str_replace("\n", "", $css);
...
if (!function_exists("wpnetbase_x2")) {
    function wpnetbase_x2($url)
    {
...
if (!function_exists("wpnetbase_in_array_r")) {
    function wpnetbase_in_array_r($needle, $haystack, $strict = false) {
        foreach ($haystack as $item) {
            if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && wpnetbase_in_array_r($needle, $item, $strict))) {
                return true;
            }
        }
...
if (!function_exists("wpnetbase_css_compress")) {
    function wpnetbase_css_compress ($code) {
...
if (!function_exists("wpnetbase_get_all_taxonomies")) {
    function wpnetbase_get_all_taxonomies() {
        $args = array(
            'public'   => true,
            '_builtin' => false
...
if (!function_exists("wpnetbase_get_all_terms")) {
    function wpnetbase_get_all_terms() {
        $taxonomies = wpnetbase_get_all_taxonomies();
        $terms = array();
        $_terms = array();
...
if (!function_exists("wpnetbase_get_all_term_ids")) {
    function wpnetbase_get_all_term_ids() {
        $taxonomies = wpnetbase_get_all_taxonomies();
        $terms = array();
        foreach ($taxonomies as $taxonomy) {
            $_temp = get_terms($taxonomy, 'orderby=name');
...
if (!function_exists("wpnetbase_four_to_array")) {
    function wpnetbase_four_to_array($data) {
        // 1.Top 2.Bottom 3.Right 4.Left
        preg_match("/\|\|(.*?)\|\|(.*?)\|\|(.*?)\|\|(.*?)\|\|/", $data, $matches);
        // 1.Top 3.Right 2.Bottom 4.Left
        return array(
...
if (!function_exists('asl_icl_t')) {
    function asl_icl_t($name, $value) {
        if (function_exists('icl_t')) {
            return stripslashes( esc_html( icl_t('nbt-ajax-search', $name, $value)) );
        }
...
if (!function_exists("wpnetbase_ismobile")) {
  function wpnetbase_ismobile() {
    $is_mobile = '0';    
...
if (!function_exists("wpnetbase_hex2rgb")) {  
  function wpnetbase_hex2rgb($color)
  {
      if (strlen($color)>7) return $color;
...
if (!function_exists("wpnetbase_rgb2hex")) {
    function wpnetbase_rgb2hex($color)
    {
        if (strlen($color)>7) {
...
if (!function_exists("wpnetbase_utf8safeencode")) {  
  function wpnetbase_utf8safeencode($s, $delimiter)
  {
...
if (!function_exists("wpnetbase_utf8safedecode")) {  
  function wpnetbase_utf8safedecode($s, $delimiter)
  {
...
if (!function_exists("wpnetbase_get_image_from_content")) {
    function wpnetbase_get_image_from_content($content, $number = 0) {
...
if (!function_exists("wpnetbase_on_backend_page")) {  
  function wpnetbase_on_backend_page($pages)
  {
...
if (!function_exists("wpnetbase_on_backend_post_editor")) {
    /**
     * @return bool
     */
    function wpnetbase_on_backend_post_editor() {
