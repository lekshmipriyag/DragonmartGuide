...
class wpnetbase_product_search_form_widget extends WP_Widget {
	...
	public function widget( $args, $instance )
	{
		$args = array(
			...
			echo $form;
			if($instance['itemcart'] == 'yes' )
			{		
				global $woocommerce; 
				echo '<div class="printshop-minicart">';
				echo "".sprintf(_n('%d item', '%d item', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count); echo " | "; echo "".$woocommerce->cart->get_cart_total();
				echo '</div>';
			}
	}
	/*Widget Backend */ 
