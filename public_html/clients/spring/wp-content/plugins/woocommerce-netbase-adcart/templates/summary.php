...
<?php if($settings['adcart-numsub'] == "sub") : ?>
	<span class="nw-visible-desktop nw-visible-phone">
			<?php echo sprintf(_n('%d '.$settings['adcart-item-name'], 
			'%d '.$settings['adcart-item-name-plural'], 
			$cart_contents_count, 
			'netbasecart'), 
			$cart_contents_count);?> - <?php echo strip_tags($woocommerce->cart->get_cart_total()); ?>
	</span>
	<span class="nw-short-contents visible-tablet nw-hidden-phone">
