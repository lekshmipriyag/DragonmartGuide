...
class RevSliderFunctionsWP {
	 ...
	 * get content url
	 */
	public static function getUrlUploads($useCommonMediaFolder = false){
	
		if(self::isMultisite() == false){	//without multisite
			$baseUrl = content_url($useCommonMediaFolder)."/";
		}
		else{	//for multisite
			$arrUploadData = wp_upload_dir($useCommonMediaFolder);
	...
	public static function import_media($file_url, $folder_name) {
		...
		if(!file_exists($ul_dir['basedir'].'/'.$artDir)) wp_mkdir_p($ul_dir['basedir'].'/'.$artDir);
		if(!file_exists($ul_dir['basedir'].'/'.$artDir.$folder_name)) wp_mkdir_p($ul_dir['basedir'].'/'.$artDir.$folder_name);
	...
	public static function getImageUrlFromPath($pathImage){
        ...
        if(empty($pathImage) || strpos($pathLower, "http://") !== false || strpos($pathLower, "https://") !== false || strpos($pathLower, "www.") === 0)
			return($pathImage);
