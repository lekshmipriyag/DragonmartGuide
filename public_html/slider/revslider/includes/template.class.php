...
class RevSliderTemplate {
	...
	public function getThemePunchTemplateSliders(){
		...
		if (version_compare($latestVersion, RevSliderGlobals::SLIDER_REVISION, '<')) {
		    $latestVersion = RevSliderGlobals::SLIDER_REVISION;
		}

		$availablePlugins = array();
		foreach ((array) get_option('revslider-addons') as $plugin) {
	...
	public function write_import_template_markup($template){
		...
		if($template['img'] == ''){
			$template['img'] = RS_PLUGIN_URL . self::DEFAULT_IMAGE;
		}
