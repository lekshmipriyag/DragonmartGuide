    ...
    private function duplicateSlider($title = false, $prefix = false){
		...
		if($title === false){
            ...
            if($prefix !== false){
                ...
                $newSliderAlias = strtolower(sanitize_title_with_dashes($title.' '.$params['title']));
            }else{
                $newSliderTitle = sanitize_text_field($title);
                $newSliderAlias = strtolower(sanitize_title_with_dashes($title));
            }
        ...
        try{
            ...
            if($is_template !== false){ //duplicate the slider now, as we just imported the "template"
				if($single_slide !== false){ //add now one Slide to the current Slider
                    ...
                    $real_slider_id = $mslider->duplicateSliderFromData(array('sliderid' => $sliderID, 'title' => $title));
				}
			}
