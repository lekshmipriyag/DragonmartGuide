      ...
      <form role="form" style="margin-bottom:20px;" name="login_form" id="login_form" method = "post" enctype="multipart/form-data" action="userlogin.php" >
        ...
        <div class="form-group">
          ...
          <input class="form-control" name="txt_secondName" type="text" />
		   ...
		   <div class="form-group">
         	 <label for="mobile"> Mobile No <span class="text-info Rhint">(eg. +971501234567, 0097141234567, 0501234567 or 041234567)</span> </label>
...
?>
              <?php
					...
					$shop_result = mysqli_query( $db, "select * from `shop_details`");
	              $shopRow = mysqli_fetch_array( $shop_result );
			 if(isset($shopRow['shop_category'])){$shopCategory = $shopRow['shop_category'];}else{$shopCategory = array();}
				$shopCategory = json_decode($shopCategory, true);
				if(!isset($shopCategory)){$shopCategory = array();}
				//print_r($shopCategory);
			  $RPstyl = "";
			  $comemain2 = "";
			  ...
			  foreach($filteredArray1 as $cate_full_array1){
				  ...
				  foreach($filteredArray2 as $cate_fmain2){
					  ...
					  if($cate_fmain2['cate_main'] != '999' && ($comemain2 == "" || $comemain2 != $cate_fmain2['cate_main'])){
						  ...
						  echo "<li><div ".$RPstyl."> + </div> <input class='parent1' name='cate[]' type='checkbox' value='".$cate_fmain2['id']."' ";
						  
						  
						  ?>
						  <?php if(array_key_exists($cate_fmain2['id'], $shopCategory)){echo "checked";} ?>
						  <?php echo " > ";
					  ...
					  foreach($filteredArray3 as $cate_flist2){
						  if($cate_flist2['cate_list'] != "999" && $cate_flist2['cate_main'] = $cate_fmain2['cate_main'] && $cate_flist2['cate_parent'] = $cate_fmain2['cate_parent']){ 
							  ...
							  echo "<input name='cate[]' class='parent1' type='checkbox' value='".$cate_flist2['id']."'";
							  ?>
							<?php if(array_key_exists($cate_flist2['id'], $shopCategory)){echo " checked ";} ?>
							<?php
...
<!-- row 3 end -->
    ...
    <script>
		$('#submit').on("click",function(){ // category validation
			 if (!jQuery(".parent1").is(":checked")) {
